...
TWrkr genAlumRan(TWrkr alum[], int alumnos);
...
void addData(TWrkr alum[], int alumnos);
...
void archivoCopias(TWrkr vect[], int alumnos);
...
void addData(TWrkr alum[], int alumnos)
{
    ...
    {
        fseek(fa, 0, SEEK_END);
        fwrite(&alum[alumnos], sizeof(TWrkr), 1, fa);
        x++;
        fclose(fa);
    }
}
...
void archivosCopias(TWrkr vect[], int n)
{
    FILE *fa;
